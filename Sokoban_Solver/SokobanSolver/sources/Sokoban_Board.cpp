...
: cost_matrix(1,1)
{
    ...
    this->populate_neighbours();
    for(auto &the_box : this->board_boxes)
        the_box.first->remove_box();
    //Mark fields outside map as walls.
    this->calc_reachable(Move_Direction::none);
    for(auto &the_row : this->board)
        for(auto &the_box : the_row)
        {
            if(!this->is_reachable(&the_box) && this->board_boxes.count(&the_box) == 0)
                the_box.type = Wall;
        }
    for(auto &the_box : this->board_boxes)
        the_box.first->insert_box();
    this->calc_reachable(Move_Direction::none);
